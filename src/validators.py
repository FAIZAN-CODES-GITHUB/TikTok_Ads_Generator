...
class FieldValidator:
    ...
    def validate_campaign_name(name: str) -> Tuple[bool, str]:
        ...
        if not name or not name.strip():
            return False, "❌ Campaign name must be provided and not empty"
        
        if len(name.strip()) < 3:
            return False, f"❌ Campaign name must have at least 3 characters (current: {len(name.strip())})"
        
        return True, "✅ Campaign name is valid"
    ...
    def validate_ad_text(text: str) -> Tuple[bool, str]:
        ...
        if not text or not text.strip():
            return False, "❌ Ad text must be provided cannot be empty"
    ...
    def validate_cta(cta: str) -> Tuple[bool, str]:
        ...
        if not cta or not cta.strip():
            return False, "❌ CTA must be provided and cannot be empty"
